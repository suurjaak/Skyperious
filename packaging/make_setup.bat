:: Creates NSIS setup file for 32-bit executable, assumed to be in working
:: directory, named skyperious_%conf.Version%.exe. Can be given in
...
set EXEFILE=%INITIAL_DIR%\skyperious_%VERSION%.exe
cd %SETUPDIR%
if not exist "%EXEFILE%" echo %EXEFILE% missing. && goto :EOF
set DESTFILE=skyperious_%VERSION%_setup.exe
echo Creating installer for Skyperious %VERSION%.
...
"%NSISDIR%\makensis.exe" /DPRODUCT_VERSION=%VERSION% "%SETUPDIR%\exe_setup.nsi"
