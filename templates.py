...
import conf, images, skypedata, util
%>
<!DOCTYPE HTML><html>
<head>
  <meta http-equiv="Content-Type" content="text/html;charset=utf-8" />
  <meta name="Author" content="{{conf.Title}}">
  <title>History of Skype {{chat["title_long_lc"]}}</title>
...
%endif
  ...
  <script>
    var highlighted_link = undefined;
    var highlighted_word = "";
    ...
    function highlight(keyword, elements) {
      ...
      for (var i = 0; i < last; i++) {
        ...
        if ("undefined" != typeof(cn)) {
          if (3 == cn.nodeType) {
            addHighlight(keyword, cn);
          } else if (0 < cn.childNodes.length) {
            highlight(keyword, cn.childNodes);
          }
        }
      }
    }

    function addHighlight(keyword, container) {
      var val = container.nodeValue;
      if (-1 == val.toLowerCase().indexOf(keyword)) return;
      var words_lc = val.toLowerCase().split(" ");
      var words = val.split(" ");
      var last = words_lc.length;
      var r = "";
      var found = false;
      for (var i = 0; i < last; i++) {
        if (words_lc[i] == keyword) {
          r += "<span class='highlight'>" + words[i] + "</span> ";
          found = true;
        } else {
          r += words[i] + " ";
        }
      ...
      if (!found) return;
      var d = document;
      var n = d.createElement("text");
      n.innerHTML = r;
      try {
        var p = container.parentNode;
        p.replaceChild(n, container);
      } catch (e) {}
    }
    ...
    function unhighlight(container) {
      if (container) {
        ...
        var re = new RegExp("<span class=['\\"]highlight['\\"]>(.*?)<\/span>", "ig");
        content = content.replace(re, "$1"); 
        container.innerHTML = content;
      }
    }

    function hilite(link, word) {
      if (highlighted_link) {
        var container = document.getElementById("content_table");
        unhighlight(container);
        highlighted_link.title = "Highlight '" + highlighted_word + "' and go to first occurrence";
      }
      if (word != highlighted_word) {
        var elements = document.getElementsByClassName("message_content");
        highlight(word.toLowerCase(), elements);
        var highlights = document.getElementsByClassName("highlight");
        if (highlights.length) {
          highlights[0].scrollIntoView();
        }
        link.title = "Unhighlight '" + word + "'";
        highlighted_word = word;
        highlighted_link = link;
      } else {
        highlighted_word = "";
        highlighted_link = undefined;
        link.title = "Highlight '" + word + "' and go to first occurrence";
      }
      return false;
    }
...
%for word, count, size in stats["wordcloud"]:
      <span style="font-size: {{sizes[size]}}"><a title="Highlight '{{word}}' and go to first occurrence" href="#" onClick="return hilite(this, '{{word}}');">{{word}}</a> ({{count}})</span> 
%endfor
    </div>
%endif
...
%endif
  <tr>
    ...
    <td class="t3"></td>
    <td class="message_content">{{parser.parse(m, html={"w": -1, "export": True})}}</td>
...
%>
%if counts["contacts"] <= 1 and result_count > 1:
...
import conf, skypedata
%>
%if counts["messages"] <= 1 and result_count > 1:
<tr><td colspan='3'><hr /></td></tr>
%endif
<tr>
  <td align="right" valign="top">
